...
install() {
  ...
  echo Compiling and installing time...
  cd ${filename}
...
setup_env() {
  ...
  local time_path=${target}/bin

  export PATH=$PATH:${time_path}
  local path_string_cmt="# Add location of time to path"
  local path_string="export PATH=\$PATH:${time_path}"
  profile=""
  ...
  if [ ! -z ${profile} ]; then 
    path_txt=`grep -o \"${time_path}\" ${profile}`

    if [ -z "${path_txt}" ]; then
      echo "Adding \"${time_path}\" to path in ${profile}" 
...
if [ ! -z "${res}" ]; then
  echo time already installed at \"${res}\".
  exit 0
fi
