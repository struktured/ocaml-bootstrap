...
let url_default  = "http://www.cmake.org/files/v3.2/cmake-3.2.2.tar.gz"
let url =
  let doc = "Url to fetch cmake sources from" in
  Arg.(value & opt string url_default & info ["u";"url"] ~doc
         ~docv:"URL")

let print s = Printf.printf "[install_cmake]: %s\n" s
...
let make ~target dir =
  ...
  Shell.system (FilePath.concat FilePath.current_dir "bootstrap") >>=
  fun res -> ignore(res); Shell.system @@ "./configure --prefix=" ^ target >>=
...
let cmd =
  let doc = "Compile and install cmake." in
  Term.(ret (pure run $ target $ url)),
  Term.info "install_cmake" ~version:"1.0" ~doc 
